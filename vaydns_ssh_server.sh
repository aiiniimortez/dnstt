...
process_domain() {
  ...
  OUTFILE=$(mktemp)
  if ! sudo dnstm tunnel add -t "$tunnel" --transport "$TUNNEL_TRANSPORT" --backend "$TUNNEL_BACKEND" --domain "$domain" --mtu "$MTU" --record-type "$RECORD_TYPE" --idle-timeout 15s --keepalive 3s --clientid-size 2 --queue-size 1024 --kcp-window-size 0 --log-level info &> "$OUTFILE"; then
